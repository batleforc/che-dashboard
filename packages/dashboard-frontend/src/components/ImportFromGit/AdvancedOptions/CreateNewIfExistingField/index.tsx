...
export class CreateNewIfExistingField extends React.PureComponent<Props, State> {
  ...
  public render() {
    ...
    return (
      <FormGroup
        label={createNewIfExisting ? 'Create New If Existing On' : 'Create New If Existing Off'}
      >
        <Switch isChecked={createNewIfExisting} onChange={value => this.handleChange(value)} />
      </FormGroup>
    );
  }
}
